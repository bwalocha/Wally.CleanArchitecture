...
namespace Wally.CleanArchitecture.Persistence
{
	public abstract class Repository<TAggregateRoot> : IRepository<TAggregateRoot> where TAggregateRoot : AggregateRoot
	{
		...
		protected IQueryable<TAggregateRoot> GetReadWriteEntitySet() => WithIncludes(_context.Set<TAggregateRoot>());

		public Task<bool> ExistsAsync(Guid id, CancellationToken cancellationToken)
		{
			return GetReadOnlyEntitySet().AnyAsync(a => a.Id == id, cancellationToken);
		}

		public Task<TAggregateRoot> GetAsync(Guid id, CancellationToken cancellationToken)
		{
			var task = GetReadWriteEntitySet().SingleAsync(a => a.Id.Equals(id), cancellationToken);
		...
		{
			var query = GetReadOnlyEntitySet().Where(a => a.Id.Equals(id));
		...
		public Task<PagedResponse<TResponse>> GetAsync<TRequest, TResponse>(ODataQueryOptions<TRequest> queryOptions, CancellationToken cancellationToken)
		...
		protected async Task<PagedResponse<TResponse>> GetAsync<TRequest, TResponse>(IQueryable<TAggregateRoot> query, ODataQueryOptions<TRequest> queryOptions, CancellationToken cancellationToken)
			where TRequest : class, IRequest
			where TResponse : class, IResponse
		{
			var queryOptionsFilter = queryOptions.Filter.ToFilterExpression<TResponse>(HandleNullPropagationOption.False);
			
			var allItems = _mapper.ProjectTo<TResponse>(queryOptionsFilter != null ? query.Where(_mapper.MapExpression<Expression<Func<TAggregateRoot, bool>>>(queryOptionsFilter)) : query);
			var items = await _mapper
				.ProjectTo<TResponse>(query.GetQuery(_mapper, queryOptions))
		...
		private Task<TResult> MapExceptionAsync<TResult>(Task<TResult> task, Guid id = default, CancellationToken cancellationToken = default)
		{
			if (task == null)
			{
			...
			task.ContinueWith(t => tcs.TrySetCanceled(), cancellationToken, TaskContinuationOptions.OnlyOnCanceled, TaskScheduler.Default);
			task.ContinueWith(t => tcs.TrySetResult(t.Result), cancellationToken, TaskContinuationOptions.OnlyOnRanToCompletion, TaskScheduler.Default);
			task.ContinueWith(
				t =>
				{
