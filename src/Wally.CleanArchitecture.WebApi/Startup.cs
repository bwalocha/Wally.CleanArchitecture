...
namespace Wally.CleanArchitecture.WebApi
{
	public class Startup
	{
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			services.AddControllers(
				...
				.AddOData(
					options =>
					{
						options.Filter().OrderBy().Count().SetMaxTop(1000);
					})
				.AddNewtonsoftJson(
					options =>
					{
			...
			dbContextOptions = options =>
			{
				options.UseSqlServer(
					...
					builder =>
					{
						...
						builder.MigrationsAssembly(typeof(Wally.CleanArchitecture.Persistence.SqlServer.Migrations.Initial).Assembly.GetName().Name);
					});
		...
		{
			appLifetime.ApplicationStarted.Register(() => logger.LogInformation("The 'Wally.CleanArchitecture' is started"));
			appLifetime.ApplicationStopping.Register(() => logger.LogInformation("The 'Wally.CleanArchitecture' is stopping"));
			appLifetime.ApplicationStopped.Register(() => logger.LogInformation("The 'Wally.CleanArchitecture' is stopped"));
			...
			app.UseEndpoints(
				endpoints =>
				{
					...
					endpoints.MapGet(
						...
						async context =>
						{
							await context.Response.WriteAsync($"v{GetType().Assembly.GetName().Version}", context.RequestAborted);
						});
				});
