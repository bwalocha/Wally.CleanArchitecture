...
namespace Wally.CleanArchitecture.ApiGateway.Tests.ConventionTests.Extensions;
	...
	private class ConstructorVisibilityShouldBeCondition : ICondition<Class>
	{
		...
		public IEnumerable<ConditionResult> Check(IEnumerable<Class> classes, Architecture architecture)
		{
			foreach (var @class in classes)
			{
				...
				{
					var pass = _visibilities.Contains(constructor.Visibility);
			...
			foreach (var member in members)
			{
				...
				if (property.Writability == Writability.InitOnly)
				{
					...
					yield return new ConditionResult(property, false,
						$"{property.FullName} must have required setter.");
				}
				else
				{
					yield return new ConditionResult(property, false,
						$"{property.FullName} must have init-only setter.");
				}
			}
		}
